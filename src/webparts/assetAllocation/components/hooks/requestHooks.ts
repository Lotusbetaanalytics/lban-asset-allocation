...
//   }
  const assetRequest = await sp.web.lists.getByTitle(listName).items.add(formData)
  return assetRequest
}

export const fetchAssetRequest = async (id) => {
  const assetRequest = await sp.web.lists.getByTitle(listName).items.getById(id)()
  return assetRequest
}

export const updateAssetRequest = async (id, formData) => {
  const assetRequest = await sp.web.lists.getByTitle(listName).items.getById(id).update(formData)
  return assetRequest
}

export const deleteAssetRequest = async (id) => {
  const assetRequest = await sp.web.lists.getByTitle(listName).items.getById(id).delete()
  return assetRequest
}